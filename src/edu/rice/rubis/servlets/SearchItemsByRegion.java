...
import edu.rice.rubis.SB_SearchItemsByRegion;
...
public class SearchItemsByRegion extends HttpServlet
{
  ...
  private void itemList(Integer categoryId, Integer regionId, int page, int nbOfItems, ServletPrinter sp, Context initialContext) 
  {
    ...
    try 
    {
      sb_SearchItemsByRegion = (SB_SearchItemsByRegion)PortableRemoteObject.narrow(initialContext.lookup("SB_SearchItemsByRegionBean"), SB_SearchItemsByRegion.class);
    } 
    catch (Exception e)
    {
    ...
    try
    {
      ...
      else
      {
        ...
        else
        {
          ...
          sp.printItemHeader();
          sp.printItemFooter("<a href=\""+Config.context+"/servlet/SearchItemsByRegion?category="+categoryId+
      ...
      if (page == 0)
        ...
        sp.printItemFooter("<a href=\""+Config.context+"/servlet/SearchItemsByRegion?category="+categoryId+
                           "&region="+regionId+"&page="+(page-1)+"&nbOfItems="+nbOfItems+"\">Previous page</a>",
                           "<a href=\""+Config.context+"/servlet/SearchItemsByRegion?category="+categoryId+
                           "&region="+regionId+"&page="+(page+1)+"&nbOfItems="+nbOfItems+"\">Next page</a>");
    } 
    catch (Exception e) 
    {
